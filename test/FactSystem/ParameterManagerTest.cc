...
void ParameterManagerTest::_setParamWithFailureMode(MockLink::ParamSetFailureMode_t failureMode, bool expectSuccess)
{
    ...
    const int expectedRawValueChangedCount = expectSuccess ? 1 : 2;
    bool rawValueChangedCountMatches = false;
    rawValueChangedSpy.wait(500);
    ...
    if (!expectSuccess) {
        rawValueChangedSpy.clear();
        rawValueChangedSpy.wait(500);
        QCOMPARE(rawValueChangedSpy.count(), 1);
        QVERIFY(QGC::fuzzyCompare(rawValueChangedSpy[0][0].toDouble(), originalDouble, 0.00001));
    }
